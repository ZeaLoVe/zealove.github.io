...
初次接任务发现规则和想象的略有不同，除了发现bug有奖励外，执行用例并确认结果一样会有奖励（用例是平台设计好的，按着步骤执行就行了）。价格根据用例步骤的复杂程度来，大约一个用例的奖励是2欧元到5欧元之间。第一次接任务因为搞虚拟手机号不成功（textnow登录一直报错），没参加测试，大概浏览了一下，熟练工的话一次应该可以执行完2-4次左右的用例，但测试任务的席位是需要抢的，所以要时刻关注邮箱有没任务。在这次的测试结果中，测试人员提交了7个bug，但只有一个被接收了。估摸着这个钱也并不好赚。
